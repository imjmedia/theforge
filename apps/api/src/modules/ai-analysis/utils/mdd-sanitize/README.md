...
**Exports de `persist-pipeline.ts`:** `sanitizeMddAtPersist`, `prepareMddMarkdownForPersist`, `storeMddMarkdownForPersist`, `sanitizeMddForExport`, `normalizeMddFormat`, `finalizeMddDeliverable`, `applyPreDeliveryGateFixes`, `repairGarbageHeadings`, `repairManifestJsonClosing`, `demoteProseHeadingsInSections`, `stripUiUxSectionForApiOnlyMvp`. `prepareMddMarkdownForPersist` deduplica §1–§7 vía `deduplicateMddDraftSections` y aplica `repairMddFormatIssues` (`@theforge/shared-types/repair-mdd-format`) antes del cierre. `normalizeMddFormat` formatea **todas** las ocurrencias de §4 con `formatAllContratosSectionsInDraft`.
