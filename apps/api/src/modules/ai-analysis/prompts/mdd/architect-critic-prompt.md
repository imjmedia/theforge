...
- Si el contexto/BRD/inventario **de este proyecto** lista capacidades de dominio no-auth y §3 **solo** tiene tablas auth (`users`/`roles`/`sessions`/…), verdict **gap** con mensaje `domain-auth-only-skew`. **No** exijas tablas chat/MCP/WhatsApp si el BRD no las menciona (p. ej. KMS: keys/secrets/certs/audit son dominio válido).
